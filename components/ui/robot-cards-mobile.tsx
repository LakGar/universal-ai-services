...
export function RobotCardsMobile({ robots, className }: RobotCardsMobileProps) {
  ...
  const RobotCardMobile = ({
    ...
    const cardContent = (
      <div className="group relative aspect-square w-[200px] overflow-hidden rounded-2xl bg-slate-800/20 backdrop-blur-md cursor-pointer">
        ...
        <div className="absolute inset-0 overflow-hidden">
          ...
          {imageError && (
            <div className="absolute inset-0 flex items-center justify-center bg-linear-to-br from-slate-800/50 to-slate-900/50 backdrop-blur-sm">
          ...
          <div className="absolute inset-0 bg-linear-to-t from-black/80 via-black/20 to-transparent opacity-100" />
        </div>
        ...
        <div className="absolute bottom-0 left-0 right-0 p-4 opacity-100">
          ...
          <div className="flex items-center gap-1 px-4 py-2 bg-transparent text-white rounded-full text-xs font-medium hover:text-blue-500 transition-colors whitespace-nowrap pointer-events-none">
            Learn more
    ...
    return (
      ...
      >
        {robot.url ? (
          <Link href={robot.url} className="block">
            {cardContent}
          </Link>
        ) : (
          cardContent
